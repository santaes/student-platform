"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LessonProgress = void 0;
const typeorm_1 = require("typeorm");
const swagger_1 = require("@nestjs/swagger");
const user_entity_1 = require("./user.entity");
const lesson_entity_1 = require("./lesson.entity");
const lesson_entity_2 = require("./lesson.entity");
let LessonProgress = class LessonProgress {
};
exports.LessonProgress = LessonProgress;
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Progress ID' }),
    (0, typeorm_1.PrimaryGeneratedColumn)(),
    __metadata("design:type", Number)
], LessonProgress.prototype, "id", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Progress status', enum: lesson_entity_2.LessonStatus }),
    (0, typeorm_1.Column)({
        type: 'varchar',
        default: lesson_entity_2.LessonStatus.LOCKED,
    }),
    __metadata("design:type", String)
], LessonProgress.prototype, "status", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Progress percentage' }),
    (0, typeorm_1.Column)({ default: 0 }),
    __metadata("design:type", Number)
], LessonProgress.prototype, "progressPercentage", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Time spent in minutes' }),
    (0, typeorm_1.Column)({ default: 0 }),
    __metadata("design:type", Number)
], LessonProgress.prototype, "timeSpentMinutes", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Started date' }),
    (0, typeorm_1.Column)({ nullable: true }),
    __metadata("design:type", Date)
], LessonProgress.prototype, "startedAt", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ description: 'Completed date' }),
    (0, typeorm_1.Column)({ nullable: true }),
    __metadata("design:type", Date)
], LessonProgress.prototype, "completedAt", void 0);
__decorate([
    (0, typeorm_1.CreateDateColumn)(),
    __metadata("design:type", Date)
], LessonProgress.prototype, "createdAt", void 0);
__decorate([
    (0, typeorm_1.UpdateDateColumn)(),
    __metadata("design:type", Date)
], LessonProgress.prototype, "updatedAt", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => user_entity_1.User, (user) => user.lessonProgress),
    __metadata("design:type", user_entity_1.User)
], LessonProgress.prototype, "student", void 0);
__decorate([
    (0, typeorm_1.ManyToOne)(() => lesson_entity_1.Lesson, (lesson) => lesson.lessonProgress),
    __metadata("design:type", lesson_entity_1.Lesson)
], LessonProgress.prototype, "lesson", void 0);
exports.LessonProgress = LessonProgress = __decorate([
    (0, typeorm_1.Entity)('lesson_progress'),
    (0, typeorm_1.Unique)(['student', 'lesson'])
], LessonProgress);
//# sourceMappingURL=lesson-progress.entity.js.map